...
const DisplayChart = ({ jobId }: DisplayChartProps) => {
  ...
  const fetchData = async () => {
    const response = await fetch(
      `https://jobed-connect-api.jobtechdev.se/v1/enriched_occupations?occupation_id=${jobId}&include_metadata=true`
    );
  ...
  const data = {
    labels: chartData?.metadata.enriched_candidates_term_frequency.competencies
      .map((competency) => competency.term)
      .slice(0, 10),
    datasets: [
      {
        label: chartData?.occupation_label,
        data: chartData?.metadata.enriched_candidates_term_frequency.competencies
          .map((competency) => competency.percent_for_occupation)
          .slice(0, 10),
        backgroundColor: ["rgba(254,192,0, 0.7)", "rgba(255,230,153, 0.7)"],
        borderColor: ["rgba(254,192,0, 1)", "rgba(255,230,153, 1)"],
        borderWidth: 3,
        hoverBorderWidth: 4,
        borderRadius: 8,
      },
